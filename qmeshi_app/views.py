...
from qmeshi_app.forms import BookForm, ImpressionForm

# urls.pyから呼ばれてページを表示するまでの処理
...
def book_list(request):
    ...
    return render(request,'qmeshi_app/book_list.html', {'books': books})      # 使用するテンプレート，テンプレートに渡すデータ    
...
def book_edit(request, book_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():    # フォームのバリデーション
            ...
            book.save()
            return redirect('qmeshi_app:book_list')
    else:    # GET の時
        form = BookForm(instance=book)  # book インスタンスからフォームを作成

    return render(request, 'qmeshi_app/book_edit.html', dict(form=form, book_id=book_id))
...
def impression_edit(request, book_id, impression_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():    # フォームのバリデーション
            ...
            impression.save()
            return redirect('qmeshi_app:impression_list', book_id=book_id)
    else:    # GET の時
        form = ImpressionForm(instance=impression)  # impression インスタンスからフォームを作成

    return render(request, 'qmeshi_app/impression_edit.html', dict(form=form, book_id=book_id, impression_id=impression_id))
